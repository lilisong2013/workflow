...
$.ligerui.controls.Grid.prototype.showFilter = function ()
{
    ...
    var filtercontainer = $('<div id="' + g.id + '_filtercontainer"></div>').width(580).height(320).hide(); 
    var filter = filtercontainer.ligerFilter({ fields: getFields() });
    return g.winfilter = $.ligerDialog.open({
        width: 700, height: 400,
        target: filtercontainer, isResize: true, top: 60,
        buttons: [
            { text: 'OK', onclick: function (item, dialog) { loadData(); dialog.hide(); } },
            { text: 'Cancel', onclick: function (item, dialog) { dialog.hide(); } }
            ]
    }); 
