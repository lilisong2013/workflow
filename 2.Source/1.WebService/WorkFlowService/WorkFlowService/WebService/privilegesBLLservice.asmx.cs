...
namespace Saron.WorkFlowService.WebService
{
    ...
    public class privilegesBLLservice : System.Web.Services.WebService
    {
        ...
        [SoapHeader("m_securityContext")]
        [WebMethod(Description = "获得某系统的所有权限下菜单列表，<h4>（需要授权验证，系统管理员）</h4>")]
        public DataSet GetMListByAppID(int appID,out string msg)
        {
            //对webservice进行授权验证,系统管理员才可访问
            if (!m_securityContext.AdminIsValid(m_securityContext.UserName, m_securityContext.PassWord, out msg))
            {
            ...
            return m_privilegesDal.GetMeListByAppID(appID);
        }

        [SoapHeader("m_securityContext")]
        [WebMethod(Description = "获得某系统的所有权限下操作列表，<h4>（需要授权验证，系统管理员）</h4>")]
        ...
        [SoapHeader("m_securityContext")]
        [WebMethod(Description = "获得某系统的所有权限下元素列表，<h4>（需要授权验证，系统管理员）</h4>")]
